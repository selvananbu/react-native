...
import OpenApiClient_prod from './openapi/openapiclient_prod_view.js';
...
var objprodView = new OpenApiClient_prod('http://swpdmsrv4.lisec.internal:18704', 'DEMO', 'PROD');
class LiChartScreen extends Component {
  ...
  callbackWithArgforDataSources(responseData){
    if(responseData !== null && responseData.state.response.data !== undefined)
    {
      ...
      obj.GET_charts(this.callbackWithArgForReports.bind(this));
    }
  }
  ...
  onPress = () => {
         ...
         Actions.LiChartViewer({
                 ...
                 fileName:this.state.criteria.name
           });
  ...
  callBackForFileDownload(responseData){
    // console.log(responseData,responseData.state.response.data);
    if( responseData.state.response.data !== undefined){
      ...
      // console.log("###############",final,responseDataString);
      Actions.LiChartViewer({
              content:final
        });
    }
    ...
    //   var path =  PictureDir + "/"+ fileName + ".png";
    //   console.log("Pathhh",path);
    //   MenuExample.showDownloadToast(path,fileName + ".png");
    // }
  }
  ...
  render() {
    ...
    return (
      ...
      <View>
        <Text style={styles.items}>Charts</Text>
        <View style={{margin:5,borderWidth:1, borderColor:'#881b4c',borderRadius:15}}>
    ...
    <TouchableOpacity style={{height:height(8),width:width(90),borderRadius:15,borderColor:'#881b4c',flexDirection: 'row',justifyContent:'center',alignItems:'center',borderWidth:2}}
      ...
      <Text style={styles.loadText}>VIEW</Text>
    </TouchableOpacity>
  </View>
</View>
);
